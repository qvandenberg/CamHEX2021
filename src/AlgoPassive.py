...
class AlgoPassive:
    ...
    def run(self):
        ...
        while (True): # test
            ...
            for instr in self.instruments:
                if self.rm.allowed_to_mutate_orders() and abs(self.rm.positions[instr]) >= 250:
                    if (self.rm.positions[instr] < -250): # buy some to restore position
                        best_bid, dummy = self.pb.get_best_bid_price_volume(instr)
                        self.book_ioc_transaction({'instrument': instr, 'price': best_bid, 'volume': 20, 'side': 'bid'})
                        print("Placing restoring order at price %.2f." %(best_bid))
                    elif (self.rm.positions[instr] > 250): # sell some to restore position
                        best_ask, dummy = self.pb.get_best_ask_price_volume(instr)
                        self.book_ioc_transaction({'instrument': instr, 'price': best_ask, 'volume': 20, 'side': 'ask'})
                        print("Placing restoring order at price %.2f." %(best_ask))
            ...
            if len(trade_opportunities) > 0 and self.rm.allowed_to_mutate_orders():
                for instr, param in trade_opportunities.items():
                    if abs(self.rm.positions[instr] < 250):
                        if param['side'] == 'bid':
